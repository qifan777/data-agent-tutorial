...
* 🛡️ **全自动纠错闭环**：自带 SQL 执行沙盒与 Python 高阶分析沙盒。遇到语法或数据错误自动回溯修复，输出即用。
...
[用户自然语言提问] 
   └── A2A 协议流式请求 
        └── 🚦 路由意图识别
             ├── 📚 知识召回 (向量化业务词汇 + QA)
             ├── 🗄️ 表结构召回 (Schema & Foreign Key 补全)
             ├── ⚖️ 可行性评估与任务拆解 (Planner)
             ├── ⏸️ 人工确认拦截 (HITL)
             ├── 💻 SQL 生成与自纠错执行沙盒 (Self-Correction)
             ├── 🐍 Python 深度数据分析与图表生成
...
* 🔥 **[04 SQL Agent 核心编排](https://www.jarcheng.top/project/data-agent/04-sql-agent-orchestration/)** *(系列高潮)*
    * 逐个击破：知识召回、Schema Linking、任务拆解、SQL 自纠错沙盒、Python 高阶计算以及商业报告生成！
